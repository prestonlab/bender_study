#!/bin/bash
#
# Run small volume correction on searchlight permutation results.

if [[ $# -lt 3 ]]; then
    echo "Run small volume correction on searchlight permutation results."
    echo
    echo "Usage:   bender_apply_clustsim_res.sh [-o] filepath model maskname"
    echo "Example: bender_apply_clustsim_res.sh mvpa/cat_react_item_rt study_stim b_par"
    echo
    exit 1
fi
...
if [[ $recalc = true ]]; then
    ...
    if [[ $(imtest "$statdir"/bg_image) = 0 ]]; then
        ln -sf "$STUDYDIR"/gptemplate/highres_brain_all/gp_template_mni_affine.nii.gz "$statdir"/bg_image.nii.gz
    fi

    # get uncorrected clusters of at least minimal size (useful for
    # defining masks, getting cluster size)
    "$FSLDIR"/bin/cluster -i "$statdir"/vox_mask -t 0.5 --minextent=100 -o "$statdir"/cluster_mask100 > "$statdir"/cluster100.txt
fi
...
"$FSLDIR"/bin/cluster -i thresh_cope1 -c cope1 -t 0.0001 --minextent="$clust_extent" --othresh=cluster_thresh_cope1 -o cluster_mask_cope1 --connectivity=26 --mm --olmax=lmax_cope1_std.txt --scalarname=Z > cluster_cope1_std.txt
