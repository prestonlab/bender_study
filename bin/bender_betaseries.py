#!/usr/bin/env python
#
# Estimate betaseries model for one run.
...
parser = ArgumentParser(
    description="Estimate betaseries using LS-S regression (Mumford et al. 2014)."
)
parser.add_argument(
    "modelbase", type=str, help="path to model files, without file extension"
)
parser.add_argument(
    "betadir", type=str, help="path to directory in which to save betaseries image"
)
parser.add_argument("ntrials", type=int, help="number of trials to be estimated")
parser.add_argument(
    "-m",
    "--mask",
    default=None,
...
for i, ev in enumerate(trial_evs):
    ...
    if deriv and args.sep_derivs:
        ...
        # for this trial
        dm_trial = N.hstack(
            (desmat.mat[:, ev, N.newaxis], desmat.mat[:, deriv_evs[i], N.newaxis])
        )
    else:
...
for i in range(len(glm_res_full)):
    ...
    ni.to_filename(os.path.join(betadir, "ev{:03d}.nii.gz".format(i)))
