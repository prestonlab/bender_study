#!/bin/bash
#
# Transform functional-space images to template space.

if [[ $# -lt 3 ]]; then
    cat <<EOF
Usage:   bender_func2mni.sh [-a anat] [-n interp] [-p postmask] input output subject
Example: bender_func2mni.sh bender_01_mat_item_w2v_faces.nii.gz bender_01_mat_item_w2v_faces_mni.nii.gz bender_01
...
if [[ $ntp -gt 1 ]]; then
    antsApplyTransforms -d 3 -e 3 -i "$input" -o "$output" -r "$reference" \
        -n "$interp" -t "$orig2temp_warp" -t "$orig2temp" -t ["${anat2func}".txt,1]
else
    antsApplyTransforms -d 3 -i "$input" -o "$output" -r "$reference" \
        -n "$interp" -t "$orig2temp_warp" -t "$orig2temp" -t ["${anat2func}".txt,1]
fi
