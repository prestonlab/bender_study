#!/usr/bin/env python
#
# Register anatomical images between days.
...
for img_name in img_names:
    ...
    if img_name == "coronal":
        if merge:
            log.run("merge_anat.sh -c %s %s %s" % (image1, image2, merged))
        else:
            log.run("N4BiasFieldCorrection -i {} -o {}".format(image1, merged))
        log.run("bet {} {} -f 0.01".format(merged, mask))
    else:
        if merge:
            log.run("merge_anat.sh %s %s %s" % (image1, image2, merged))
        else:
            log.run("N4BiasFieldCorrection -i {} -o {}".format(image1, merged))
...
log.run(
    "antsRegistration -d 3 -r [{highres},{coronal},1] -t Rigid[0.1] -m MI[{highres},{coronal},1,32,Regular,0.25] -c [1000x500x250x100,1e-6,10] -f 8x4x2x1 -s 3x2x1x0vox -n BSpline -w [0.005,0.995] -o {xfm}".format(
...
if not merge:
    ...
    log.run(
        "antsApplyTransforms -i {input} -o {output} -r {ref} -t {c2h} -n BSpline".format(
    ...
    log.run(
        "antsApplyTransforms -i {input} -o {output} -r {ref} -t {c2h} -t {c2c} -n BSpline".format(
            input=coronal1, output=coronal1_reg, ref=highres_merge, c2h=c2h, c2c=c2c
        )
    )

    # merge registered coronals
    log.run("merge_anat.sh -m {} {} {}".format(coronal1_reg, coronal2_reg, coronal_reg))
