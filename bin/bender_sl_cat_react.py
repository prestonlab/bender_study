...
parser.add_argument(
    "--division", "-d", default="cat", help="category labels to use ([cat],subcat)"
)
parser.add_argument(
    "--rec",
    "-e",
...
if "svm" in args.measure:
    ...
    ds = bp.block_dataset("c_loc", 2.0, mask=mask_file, shift=4.0, suffix="_hpfsm")
...
if args.measure == "svm":
    ...
    m = traintest.TrainTest(
        clf, "dataset", ds2.sa.chunks, n_perm=args.n_perm, output=args.output
    )
elif args.measure == "svm_sme":
    # AUC for category coding, by subsequent memory
    clf = LinearCSVMC(probability=1, enable_ca=["probabilities"])
    m = traintest.TrainTestSME(
        clf,
        ds_full.sa.dataset,
        ds2.category,
        correct,
    ...
    m = sim_react.SimCatReact(
        ds_full.sa.dataset, ds2.sa.category, n_perm=args.n_perm, output=args.output
    )
elif args.measure in ["cat_sme", "cat_rt"]:
    # category separation as a function of subsequent memory or RT
    m = sim_react.SimCatReactSME(
        ds_full.sa.dataset,
        ds2.sa.category,
        correct,
        n_perm=args.n_perm,
