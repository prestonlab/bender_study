...
parser.add_argument('--measure', '-m', default='corr',
                    help="measure to calculate ([corr],sme)")
parser.add_argument('--partial', default=None, type=str,
                    help="models to partial out (models3)")
parser.add_argument('--fit', '-f', default='ls', type=str,
...
parser.add_argument('--ind-file', '-i', default=None,
                    help="MAT-file with random indices")
...
if args.phase == 'prex':
    if args.feature_mask is not None:
        feature_file = bp.image_path('anatomy', 'bbreg', 'data',
                                     args.feature_mask)
...
if args.ind_file is not None:
    ...
    if args.cat is None:
        raise IOError('If loading random indices from file, cannot use both categories.')
...
if args.measure == 'corr':
    if args.partial is not None:
        m = sim_model.SimRDMPartial2(division, rdm, control_rdms, args.n_perm,
                           fit=args.fit, output=args.output)
    else:
        m = sim_model.SimRDM(division, rdm, args.n_perm, output=args.output,
                   rand_ind=rand_ind)
    ...
    test = bp.read_period('ac_test', 5.5)
    correct = test.sort('group').array('correct')
    m = sim_model.SimRDMCatSME(division, correct[include], rdm, args.n_perm, args.output)
...
if args.output == 'roi':
    ...
    res_dir = os.path.join(bp.study_dir, 'batch', 'rsa', args.resname,
                           'roi', args.mask)
