...
class BenderPath(SubjPath):
    ...
    def read_loc_blocks(self, run):
        ...
        block_events = Events()
        block_events.append({'onset':0, 'block':1, 'category':'rest',
                             'subcategory':'rest', 'duration':18})
        ...
        for i, b in enumerate(ublocks):
            ...
            block_events.append({'onset':e['onset'], 'block':i + 2,
                                 'category':cat, 'subcategory':subcat,
                                 'duration':20})
            
        onset = block_events[-1]['onset'] + 20
        block_events.append({'onset':onset, 'block':len(ublocks)+2,
                             'category':'rest', 'subcategory':'rest',
                             'duration':18})
        return block_events
    ...
    def get_n_runs(self, period):
        ...
        # fMRI code
        d = {'p_loc':1,'p_prex':1,'c_loc':4,'a_prex':6,'p_study':1,
             'p_feedback':1,'ab_study':4,'ab_feedback':4,
             'bcxy_study':6,'ac_test':5,'bcxy_test':1,'ab_test':1,
             'loc':4,'prex':6,'study':6,'test':5}
        return d[period]
    ...
    def run_files(self, period, run=None, suffix='',
                  subdir='antsreg/data', sepdirs=False):
        ...
        files = self.bold_files(
            sepdirs=sepdirs, file_pattern=file_pattern,
            subdir=subdir, suffix=suffix)
        return files

    def block_dataset(self, period, duration, mask=None, shift=0., suffix=''):
    ...
    def beta_dataset(self, period, mask=None, suffix='_stim', include=None):
        ...
        run_period = period.split('_')[1]
        beta_dir = os.path.join(self.study_dir, 'batch', 'glm',
                                run_period + suffix, 'beta')
